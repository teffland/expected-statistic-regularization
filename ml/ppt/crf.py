...
from torch_struct.deptree import convert as _convert, unconvert as _unconvert
import torch

from ml.ppt.matrix_tree import compute_log_partitions, compute_marginals
from ml.ppt.mst import decode_mst
...
class DepTreeCRF:
    ...
    def __init__(
        self, scores: Tensor, mask: Optional[BoolTensor] = None, projective: bool = False, multiroot: bool = False,
    ) -> None:
        assert scores.dim() == 4
    ...
    def log_probs(self, heads: LongTensor, types: LongTensor, score_only: bool = False) -> Tensor:
    ...
    def argmax(self) -> Tuple[LongTensor, LongTensor]:
        ...
        if self.proj:
            ...
            if not self.multiroot:
                warnings.warn("argmax for non-projective is still multiroot although multiroot=False")
            # shape: (bsz, slen)
