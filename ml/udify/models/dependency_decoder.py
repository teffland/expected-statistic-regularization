...
# from ml.training.attachment_scores import AttachmentScores
from ml.utils.stat_funcs import coarse_dep_tags, clean_dep
...
@Model.register("udify_dependency_decoder", exist_ok=True)
class DependencyDecoder(Model):
    ...
    def __init__(
        ...
        # Create a matrix for reducing fine dep tags to coarse ones since true UDP LAS is defined over coarse ones
        # whereas the above attachment metrics will be calculated over fine ones
        fine_dep_t2i = self.vocab.get_token_to_index_vocabulary("head_tags")
        M = torch.zeros(len(fine_dep_t2i), len(coarse_dep_tags))
        for dep, i in fine_dep_t2i.items():
            # print("try map", dep, i)
            try:
                j = coarse_dep_tags.index(clean_dep(dep))
                M[i, j] = 1
                # print(f"Mapping {dep} -> {clean_dep(dep)} @ {(i,j)}")
            except Exception as e:
                # print(e)
                pass
        self.coarse_dep_tags = coarse_dep_tags
        self.dep_label_fine_to_coarse_reduce_matrix = M  # torch.nn.Parameter(M, requires_grad=False)
        self._coarse_attachment_scores = AttachmentScores()
    ...
    def forward(
        ...
        # Prepend the head (ROOT) sentinel onto the sentence representation.
        ...
        # print("encoded_text norms", torch.linalg.norm(encoded_text, dim=2))
        # print("head_arc_representation norms", torch.linalg.norm(head_arc_representation, dim=2))
        # print("child_arc_representation norms", torch.linalg.norm(child_arc_representation, dim=2))
        ...
        # !IMPORTANT!
        # This is actually a semantic/naming bug that doesn't affect the rest of the model, but it is
        # important for understanding the code.  The `attended_arcs` look like they produce a matrix of [head,child]
        # potentials, but in fact, they produce the transpose ([child,head]) _because_ they are used that way throughout
        # the rest of the model code. So in fact the `head\child_arc_representation` and `head\child_arc_feedforward`
        # models functionally perform eachother's jobs.
        #
        # TL;DR
        # This line should be refactored to `self.arc_attention(child_arc_representation, head_arc_representation)`
        # but can't for backwards compatibility reasons.
        attended_arcs = self.arc_attention(head_arc_representation, child_arc_representation)
        # print("head arc logit norms", torch.linalg.norm(attended_arcs, dim=2))

        minus_inf = -1e8
        minus_mask = (1 - float_mask) * minus_inf
        attended_arcs = attended_arcs + minus_mask.unsqueeze(2) + minus_mask.unsqueeze(1)

        head_arc_logits, head_tag_logits = [], []
        arc_logits, label_logits = [], []
        # use_greedy = not metadata[0].get("use_mst_decoding", False)

        if self.training or not self.use_mst_decoding_for_validation:
            # print("greedy")
            (predicted_heads, predicted_head_tags, head_arc_logits, head_tag_logits,) = self._greedy_decode(
        ...
        if head_indices is not None and head_tags is not None:
            ...
            evaluation_mask = self._get_mask_for_eval(mask[:, 1:], pos_tags).bool()
            ...
            # print("head idxs", head_indices[0])
            # print("pred head idxs", predicted_heads[0])
            # print("head tags", head_tags[0])
            # print("pred head tags", predicted_head_tags[0])
            # print("evaluation_mask", evaluation_mask[0])
            # print(
            #     self._attachment_scores._labeled_correct,
            #     self._attachment_scores._unlabeled_correct,
            #     self._attachment_scores._exact_labeled_correct,
            #     self._attachment_scores._exact_unlabeled_correct,
            #     self._attachment_scores._total_words,
            #     self._attachment_scores._total_sentences,
            # )
            # print(f"ATTACH:: {self._attachment_scores.get_metric(reset=True)}")
            predicted_coarse_head_tags = self.fine_to_coarse_tags(predicted_head_tags)
            gold_coarse_head_tags = self.fine_to_coarse_tags(head_tags)
            self._coarse_attachment_scores(
                predicted_heads[:, 1:],
                predicted_coarse_head_tags[:, 1:],
                head_indices[:, 1:],
                gold_coarse_head_tags[:, 1:],
                evaluation_mask,
            )
        else:
            # No gold parse given, set the decoder loss to zero
            # (I'm not sure why they had this doing some sort of self-supervised loss before)
            # arc_nll, tag_nll = self._construct_loss(
            #     head_tag_representation=head_tag_representation,
            #     child_tag_representation=child_tag_representation,
            #     attended_arcs=attended_arcs,
            ...
            # )
            # loss = arc_nll + tag_nll
            arc_nll, tag_nll, loss = 0.0, 0.0, 0.0

        for_min_risk = metadata[0].get("for_min_risk", True)
        if for_min_risk:
            arc_logits = attended_arcs.transpose(1, 2)  # shape: B, N, N (batch, head index, child index)
            label_logits = efficient_bilinear_outer(
                head_tag_representation, child_tag_representation, self.tag_bilinear
            )  # shape: B, L, N, N (head index, child index) ordering
        ...
        output_dict = {
            ...
            "head_arc_logits": head_arc_logits,
            "head_label_logits": head_tag_logits,
    ...
    def decode(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for instance_heads, instance_tags, length in zip(heads, head_tags, lengths):
            ...
            # instance_heads = [0 for _ in instance_heads]  # hack all ROOT
            # print("instance_heads", instance_heads)
    ...
    def _construct_loss(
        ...
        normalised_head_tag_logits = masked_log_softmax(head_tag_logits, mask.unsqueeze(-1)) * float_mask.unsqueeze(-1)
    ...
    def _greedy_decode(
        ...
        return heads, head_tags, head_arc_logits, head_tag_logits

    def fine_to_coarse_tags(self, fine_head_tags):
        B, N = fine_head_tags.shape
        self.dep_label_fine_to_coarse_reduce_matrix = self.dep_label_fine_to_coarse_reduce_matrix.to(
            fine_head_tags.device
        )
        M = self.dep_label_fine_to_coarse_reduce_matrix
        coarse_head_tags = M.index_select(0, fine_head_tags.reshape(-1)).max(dim=1)[1].reshape(B, N)
        return coarse_head_tags
    ...
    def _run_mst_decoding(batch_energy: torch.Tensor, lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        heads = []
        head_tags = []
        D = batch_energy.device
        for energy, length in zip(batch_energy.detach().cpu(), lengths):
            # These scores are equal to exp{ normed_arc_logit[i,j] + max_c(normed_tag_logit[c,i,j]) }
            # that is, the arc logits are actually modified by the logit of the highest scoring tag for the
        ...
        return torch.from_numpy(numpy.stack(heads)).to(D), torch.from_numpy(numpy.stack(head_tags)).to(D)

    def _get_head_tags(
        self, head_tag_representation: torch.Tensor, child_tag_representation: torch.Tensor, head_indices: torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ms = {
            f"_deps/fine_{metric_name}": metric
            for metric_name, metric in self._attachment_scores.get_metric(reset).items()
            if metric_name in ("LAS", "UAS")
        }
        ms.update(
            {
                f".run/deps/coarse_{metric_name}": metric
                for metric_name, metric in self._coarse_attachment_scores.get_metric(reset).items()
                if metric_name in ("LAS", "UAS")
            }
        )
        return ms
...
def efficient_bilinear_outer(x, y, L):
    """ Compute bilinear inner product for each possible pair i,j in N.

    That is, we get a pair of sequence reps in x = (B, N, D1), y = (B, N, D2)
    and we want a result tensor of out = (B,C,N,N)
    where out[b][c,i,j] = x_i^T W_c y_j + b_c.

    This can be done more efficiently than repeating x and y across eachothers dimensions and using bilinear directly
    by instead computing the inner product z_ic = x_i^T W_c 
    then computing the tensor product z_ic \dot y_jc w/ an implicit broadcasting.

    It takes the parallel time and memory requirements from O(CN^2) to O(CN) 
    """

    B, N, D1 = x.shape
    W = L.weight  # shape: C, D1, D2
    z = x.unsqueeze(1).matmul(W.unsqueeze(0))  # shape: B, C, N, D1
    y = y.transpose(1, 2).unsqueeze(1)  # shape: B, 1, D2, N
    out = z.matmul(y)  # shape: B, C, N, N
    if L.bias is not None:
        out = out + L.bias.reshape(1, -1, 1, 1)
    return out
