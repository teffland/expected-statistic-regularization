...
from ml.udify.dataset_readers.lemma_edit import apply_lemma_rule
...
class TagDecoder(Model):
    ...
    def __init__(
        ...
        self.metrics.update(
            {f"_acc_{tag}": BooleanAccuracy() for tag in self.vocab.get_index_to_token_vocabulary(task).values()}
        )
        ...
        if self.adaptive:
            ...
            self.task_output = AdaptiveLogSoftmaxWithLoss(
                self.output_dim, self.num_classes, cutoffs=adaptive_cutoffs, div_value=4.0
            )
        else:
        ...
        for feature in self.features:
            self.feature_outputs[feature] = TimeDistributed(Linear(self.output_dim, vocab.get_vocab_size(feature)))
    ...
    def _adaptive_loss(self, hidden, mask, gold_tags, output_dim):
        ...
        if gold_tags is not None:
            output_dict["loss"] = sequence_cross_entropy(
                class_probabilities, gold_tags, mask, label_smoothing=self.label_smoothing
            )
    ...
    def _loss(self, hidden, mask, gold_tags, output_dim):
        ...
        if gold_tags is not None:
            ...
            self.metrics["acc"](logits, gold_tags, mask.float())
            pred_tags = logits.max(-1)[-1]
            for i, tag in self.vocab.get_index_to_token_vocabulary(self.task).items():
                tag_mask = mask.float() * (gold_tags == i).float()
                self.metrics[f"_acc_{tag}"](pred_tags, gold_tags, tag_mask)
    ...
    def decode(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for predictions, words in zip(predictions_list, all_words):
            ...
            tags = [self.vocab.get_token_from_index(x, namespace=self.task) for x in argmax_indices]
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        other_metrics = {
            f"_{self.task}/{metric_name}": metric.get_metric(reset)
            for metric_name, metric in self.metrics.items()
        ...
        return {**main_metrics, **other_metrics, **features_metrics}
