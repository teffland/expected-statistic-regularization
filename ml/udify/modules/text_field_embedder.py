...
class UdifyTextFieldEmbedder(TextFieldEmbedder):
    ...
    def forward(self, text_field_input: Dict[str, torch.Tensor], num_wrapping_dims: int = 0) -> torch.Tensor:
        ...
        def embed(key):
            ...
            if self._embedder_to_indexer_map is not None:
                tensors = [text_field_input[indexer_key] for indexer_key in self._embedder_to_indexer_map[key]]
            else:
    ...
    @classmethod
    def from_params(cls, vocab: Vocabulary, params: Params, **kwargs) -> "UdifyTextFieldEmbedder":  # type: ignore
