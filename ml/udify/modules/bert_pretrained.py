...
class WordpieceIndexer(TokenIndexer):
    ...
    def tokens_to_indices(self, tokens: List[Token], vocabulary: Vocabulary) -> Dict[str, List[List[int]]]:
        ...
        if len(flat_wordpiece_ids) <= window_length:
            ...
            wordpiece_windows = [
                self._start_piece_ids + flat_wordpiece_ids[i : i + window_length] + self._end_piece_ids
        ...
        return {
            self._namespace: wordpiece_ids,
            f"{self._namespace}-offsets": offsets,
            f"{self._namespace}-type-ids": token_type_ids,
            "mask": mask,
        }
    ...
    def as_padded_tensor_dict(
        ...
        return {
            key: torch.LongTensor(pad_sequence_to_length(val, desired_num_tokens[key])) for key, val in tokens.items()
        }
...
def _get_token_type_ids(wordpiece_ids: List[int], separator_ids: List[int]) -> List[int]:
    ...
    while cursor < num_wordpieces:
        ...
        elif all(wordpiece_ids[cursor + index] == separator_id for index, separator_id in enumerate(separator_ids)):
...
class BertEmbedder(TokenEmbedder):
    ...
    def __init__(
        ...
        if self.combine_layers == "mix":
            self._scalar_mix = ScalarMixWithDropout(
                bert_model.config.num_hidden_layers, do_layer_norm=False, dropout=layer_dropout
            )
        else:
    ...
    def forward(
        self, input_ids: torch.LongTensor, offsets: torch.LongTensor = None, token_type_ids: torch.LongTensor = None
    ) -> torch.Tensor:
        """
        Parameters
        ----------
...
class UdifyPretrainedBertEmbedder(BertEmbedder):
    def __init__(
        ...
        super().__init__(bert_model=model, layer_dropout=layer_dropout, combine_layers=combine_layers)
