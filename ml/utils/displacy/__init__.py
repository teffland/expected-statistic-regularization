...
from ml.utils.displacy.render import DependencyRenderer, EntityRenderer
from spacy.tokens import Doc, Span

# from spacy.compat import b_to_str
...
def render(docs, style="dep", page=False, minify=False, jupyter=None, options={}, manual=False):
...
def render_pair(doc_pair, style="dep", page=False, minify=False, jupyter=None, options={}, manual=False):
    """Render displaCy visualisation.

    doc_pairs (dict[str, Union[Doc, Span, dict]]): pair of docs with matching tokens at keys ('g','p' or 'gold','pred)
    style (unicode): Visualisation style, 'dep' or 'ent'.
    page (bool): Render markup as full HTML page.
    minify (bool): Minify HTML markup.
    jupyter (bool): Override Jupyter auto-detection.
    ...
    if style not in factories:
        raise ValueError(Errors.E087.format(style=style))
    gold_doc = doc_pair["g"] if "g" in doc_pair else doc_pair["gold"]
    pred_doc = doc_pair["p"] if "p" in doc_pair else doc_pair["pred"]

    gold_doc = gold_doc if not isinstance(gold_doc, Span) else gold_doc.as_doc()
    pred_doc = pred_doc if not isinstance(pred_doc, Span) else pred_doc.as_doc()
    if not all(isinstance(obj, (Doc, Span, dict)) for obj in [gold_doc, pred_doc]):
        raise ValueError(Errors.E096)
    renderer, converter = factories[style]
    renderer = renderer(options=options)
    parsed_gold = converter(gold_doc, options) if not manual else gold_doc
    parsed_pred = converter(parsed_pred, options) if not manual else pred_doc
    _html["parsed"] = renderer.render_pair(parsed_gold, parsed_pred, page=page, minify=minify).strip()
    html = _html["parsed"]
    if RENDER_WRAPPER is not None:
        html = RENDER_WRAPPER(html)
    if jupyter or (jupyter is None and is_in_jupyter()):
...
def serve(
    docs, style="dep", page=True, minify=False, options={}, manual=False, port=5000, host="0.0.0.0",
):
...
def parse_deps(orig_doc, options={}):
    ...
    for word in doc:
        if word.i < word.head.i:
            arcs.append({"start": word.i, "end": word.head.i, "label": word.dep_, "dir": "left"})
        elif word.i > word.head.i:
            arcs.append(
                {"start": word.head.i, "end": word.i, "label": word.dep_, "dir": "right",}
            )
