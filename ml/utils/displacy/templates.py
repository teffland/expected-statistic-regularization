...
TPL_DEP_WORD_GP = """
<text class="displacy-token" fill="currentColor" text-anchor="middle" y="{y}">
    <tspan class="displacy-tag" style="font-size: {fontSize}; font-weight: {tagFontWeight};" fill="{tagColor}" x="{x}">{gtag}</tspan>
    <tspan class="displacy-word" style="font-size: {fontSize}; font-weight: 800;"  dy="{fontSep}" fill="currentColor" x="{x}">{text}</tspan>
    <tspan class="displacy-tag" style="font-size: {fontSize}; font-weight: {tagFontWeight};"  dy="{fontSep}" fill="{tagColor}" x="{x}">{ptag}</tspan>
</text>
"""

TPL_DEP_ARCS_GP = """
<g class="displacy-arrow">
    <path class="displacy-arc" id="arrow-{id}-{i}" stroke-width="{stroke}px" d="{arc}" fill="none" stroke="{strokeColor}"/>
    <text dy="{textOffset}px" style="font-size: {arrowFontSize}; letter-spacing: 1px">
        <textPath xlink:href="#arrow-{id}-{i}" class="displacy-label" startOffset="50%" side="{label_side}" fill="{strokeColor}" text-anchor="middle">{label}</textPath>
    </text>
    <path class="displacy-arrowhead" d="{head}" fill="{strokeColor}"/>
</g>
"""
